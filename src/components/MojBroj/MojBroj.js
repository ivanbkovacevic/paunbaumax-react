...
state={
  ...
  AddNumber=(digit)=>{
    let playerInput; 
    let {playerDisplayInput}=this.state;
    let duplo='ne moye dva puta isti broj';
    this.state.playerInput === digit ?  playerDisplayInput+=duplo : playerDisplayInput+=playerInput; 
    
    playerDisplayInput+=playerInput;
    this.setState({playerInput,playerDisplayInput});
    console.log(playerInput,typeof(playerInput));
    console.log(playerDisplayInput);
  ...
  Calculate=()=>{
      let calculation;
      calculation=eval(this.state.playerDisplayInput);
    ...
    render() {
      ...
      return(
        <div>
            ...
            <p>f</p>
            <p>f</p>
          <button onClick={this.generateTarget}>IZABERI BROJ</button><p>-</p>
          <button onClick={this.generateSolution}>POGLEDAJ  RESENJE</button>
          ...
          <button className='button-detaljnije' value={')'} onClick={()=>this.AddOperation(')')}>)</button>
            <div>
                <h2>PLAYERS INPUT</h2>
                <p>{this.state.playerDisplayInput}</p>
